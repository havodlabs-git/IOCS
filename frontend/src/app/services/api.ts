...
// ---------------------------------------------------------------------------
// Blue Team Approval Endpoints (requer Admin Key)
// ---------------------------------------------------------------------------

export interface IocRecordWithCustomer extends IocRecord {
  submittedByCustomerName?: string | null;
}

/**
 * Lista IOCs GLOBAL pendentes de aprovação Blue Team.
 */
export async function listPendingIocs(
  adminKey: string,
): Promise<{ total: number; data: IocRecordWithCustomer[] }> {
  return request<{ total: number; data: IocRecordWithCustomer[] }>(
    'GET', '/api/IOCS/blueteam/pending', undefined, undefined, adminKey,
  );
}

/**
 * Lista IOCs GLOBAL aprovados.
 */
export async function listApprovedGlobalIocs(
  adminKey: string,
): Promise<{ total: number; data: IocRecordWithCustomer[] }> {
  return request<{ total: number; data: IocRecordWithCustomer[] }>(
    'GET', '/api/IOCS/blueteam/global', undefined, undefined, adminKey,
  );
}

/**
 * Aprova um IOC PENDING.
 */
export async function approveIoc(
  iocId: string,
  adminKey: string,
  notes?: string,
): Promise<{ data: IocRecord; message: string }> {
  return request<{ data: IocRecord; message: string }>(
    'POST', '/api/IOCS/blueteam/approve', { id: iocId, notes }, undefined, adminKey,
  );
}

/**
 * Rejeita/revoga um IOC (PENDING ou APPROVED) — remove do pool global.
 */
export async function rejectIoc(
  iocId: string,
  adminKey: string,
  notes?: string,
): Promise<{ message: string; id: string }> {
  return request<{ message: string; id: string }>(
    'POST', '/api/IOCS/blueteam/reject', { id: iocId, notes }, undefined, adminKey,
  );
}
