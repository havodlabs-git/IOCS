    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md z-10">
        {!done ? (
          <div className="p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">Uma cópia será criada no escopo global</p>
              </div>
            </div>
          ...
          <div className="flex flex-col items-center py-10 px-6 text-center">
            <div className="bg-green-100 p-4 rounded-full mb-4"><CheckCircle className="w-10 h-10 text-green-600" /></div>
            <h3 className="text-lg font-bold text-slate-800 mb-2">IOC Compartilhado!</h3>
            <p className="text-sm text-slate-500 mb-6"><code className="font-mono bg-slate-100 px-2 py-0.5 rounded text-slate-700">{ioc.value}</code> agora está disponível globalmente.</p>
...
export function IocListPage({ iocs, loading, onRefresh, auth }: IocListPageProps) {
  ...
  const types    = Array.from(new Set(iocs.map((i) => i.type))).sort();
  const statuses = Array.from(new Set(iocs.map((i) => i.status ?? ''))).filter(Boolean).sort();
  const scopes   = Array.from(new Set(iocs.map((i) => i.scope ?? ''))).filter(Boolean).sort();
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            {loading ? 'Carregando...' : `${filtered.length} de ${iocs.length} IOC(s)`}
          </p>
        </div>
                ...
                {filtered.map((ioc, idx) => {
                  ...
                  return (
                    <tr key={ioc.id}
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${ioc.scope === 'GLOBAL' ? 'bg-purple-100 text-purple-700' : 'bg-teal-100 text-teal-700'}`}>
                          {ioc.scope === 'GLOBAL' ? <Globe className="w-3 h-3 mr-1" /> : null}{ioc.scope ?? '—'}
                        </span>
                      </td>
