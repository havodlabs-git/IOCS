...
import { X, Shield, CheckCircle, Copy, AlertTriangle, Loader2 } from 'lucide-react';
...
type Step = 'loading' | 'qr' | 'verify' | 'success' | 'disable' | 'disabled';
  ...
  useEffect(() => {
    if (step === 'loading') {
      setupMfa(token)
        ...
        .catch((e) => {
          setError(e.message === 'MFA_ALREADY_ENABLED' ? 'MFA já está activo.' : e.message);
          setStep('qr');
        });
    }
  }, [step, token]);
